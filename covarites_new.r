...
theta_hat<-function(p,q,n,x,y,lambda,rule=c("and","or"),penalty=c("lasso","SCAD"))
{
	...
	for(j in 1:q)
	{
		...
		esi<-ncvreg(phi_,eta,penalty = "SCAD",family="gaussian",gamma=3.7, alpha=1,lambda.min=0.001,nlambda =10,lambda,eps=.001,max.iter=1500,convex=TRUE,warn=TRUE)$beta[-1]
		dim(esi)<-c(p,q-1)
		theta_hat[j,-j,]<-t(esi)
		return(theta_hat)
	}
